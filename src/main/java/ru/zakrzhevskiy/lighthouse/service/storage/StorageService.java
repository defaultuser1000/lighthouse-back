...
public class StorageService {
    ...
    @Autowired
    @Qualifier("storageStrategy")
    private StorageStrategy storageStrategy;

    public void createFolder(Order order, String additionalPath) {
        this.storageStrategy.createFolder(order, additionalPath);
    }
    ...
    public List<StorageItemDto> listDirContent(Order order, CharSequence additionalPath) {
        ...
        if (additionalPath != null && additionalPath.length() > 0) {
            rootDir += "/" + (
                    additionalPath.toString().endsWith("/")
                            ? StringUtils.chop(additionalPath.toString())
